...
async def PRINT_RESULTS(host, port):
    ...
    print('%-28s %-10s %-20s' % (DATA_HOST, INFO_PORTS, INFO_BANNER))
    ...
    if host!=False:
        ...
        for LIST_PORT in LIST_PORT_RUN:
            if re.search(r'-', str(LIST_PORT)):
                ...
                for PORT_SPLIT in range(int(start_port), int(end_port) + 1):
